...
create table public.documents (
  ...
  name text not null,
  type text check (type in ('employer_letter', 'degree', 'transcript', 'license', 'i94', 'passport', 'other')) not null,
...
-- Companies: public read, verified users write
...
-- Work history: public read (via profile), own write
...
alter table public.documents add constraint documents_type_check
  check (type in ('employer_letter', 'degree', 'transcript', 'license', 'i94', 'passport', 'resume', 'credential_evaluation', 'support_letter', 'photo', 'other'));
...
create trigger on_job_updated before update on public.jobs
  for each row execute function public.handle_updated_at();
