...
export default function JobDetailPage() {
	...
	useEffect(() => {
		async function load() {
			...
			const { data, error } = await supabase
				.from("jobs")
				.select(
					"id, slug, title, company_id, company_name, tn_profession, description, requirements, salary_min, salary_max, location, remote_policy, employment_type, application_url, is_featured, posted_at, expires_at",
				)
				.eq("slug", slug)
				.single();
			if (error && error.code !== "PGRST116")
				logger.error("Failed to load job:", error.message);
			setJob(data);
			setLoading(false);
		}
		load();
	...
	const salary =
		job.salary_min && job.salary_max
			...
			: job.salary_min
				? `From $${(job.salary_min / 1000).toFixed(0)}K/year`
				: job.salary_max
					? `Up to $${(job.salary_max / 1000).toFixed(0)}K/year`
					: null;
		...
		<>
			...
			<div className="section-padding">
				<div className="container-tight">
					<Link
						href="/jobs"
						className="inline-flex items-center gap-1 text-sm text-accent mb-6"
					>
					...
					<div
						className={clsx(
							"card p-8",
							job.is_featured && "border-accent border-2",
						)}
					>
						{job.is_featured && (
						...
						{job.description.length < 500 && (
							...
							>
								Read full description on employer site{" "}
					...
					<div className="mt-6 card p-5">
						<p className="text-sm text-fg-secondary">
							This job requires{" "}
							<Link
								href={`/professions/${professions.find((p) => p.name === job.tn_profession)?.slug || job.tn_profession.toLowerCase().replace(/[\s/]+/g, "-")}`}
								className="text-accent hover:underline"
							>
								{job.tn_profession}
							</Link>{" "}
							TN visa classification.{" "}
							<Link href="/eligibility" className="text-accent hover:underline">
