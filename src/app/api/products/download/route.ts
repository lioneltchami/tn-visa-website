...
import { consumeDownload, getPurchaseById } from '@/lib/purchases'
...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (!product) {
      console.error('[download] Purchase references unknown product:', purchase.product_id)
      return NextResponse.json(
        {
          error: 'This product is unavailable. Please contact hello@tnvisaguide.ca.',
        },
    ...
    if (error || !data?.signedUrl) {
      console.error('[download] Signed URL failed for', file.path, error?.message)
      return NextResponse.json(
        {
          error:
