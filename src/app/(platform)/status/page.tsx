...
import { Clock, Plus, AlertCircle, CheckCircle, AlertTriangle } from 'lucide-react'
...
export default function StatusPage() {
  ...
  const [form, setForm] = useState({
    profession: '', employer: '', start_date: '', end_date: '',
    application_method: '', port_of_entry: '', notes: '',
  })
    ...
    async function load() {
      try {
        ...
        const { data, error: err } = await supabase
          .from('tn_status').select('*').eq('user_id', user.id).order('end_date', { ascending: false })
        if (err) throw err
        setStatuses(data || [])
      } catch (e) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const { data, error: err } = await supabase.from('tn_status').insert({
        user_id: user.id,
      ...
      setShowForm(false)
      setForm({ profession: '', employer: '', start_date: '', end_date: '', application_method: '', port_of_entry: '', notes: '' })
    } catch (e) {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setShowForm(!showForm)} className="flex items-center gap-2 px-4 py-2 rounded-full gradient-bg text-white text-sm font-medium">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="card p-6 mb-6 space-y-3">
          ...
          <div><label className="block text-sm font-medium text-fg-secondary mb-1">Employer *</label>
            <input required value={form.employer} onChange={e => setForm(f => ({ ...f, employer: e.target.value }))} className={inputCls} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="block text-sm font-medium text-fg-secondary mb-1">Start Date *</label>
              <input type="date" required value={form.start_date} onChange={e => setForm(f => ({ ...f, start_date: e.target.value }))} className={inputCls} />
            </div>
            <div><label className="block text-sm font-medium text-fg-secondary mb-1">End Date *</label>
              <input type="date" required value={form.end_date} onChange={e => setForm(f => ({ ...f, end_date: e.target.value }))} className={inputCls} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="block text-sm font-medium text-fg-secondary mb-1">Application Method</label>
              <select value={form.application_method} onChange={e => setForm(f => ({ ...f, application_method: e.target.value }))} className={inputCls}>
                <option value="">Select...</option>
                {METHODS.map(m => <option key={m.value} value={m.value}>{m.label}</option>)}
              </select>
            </div>
            <div><label className="block text-sm font-medium text-fg-secondary mb-1">Port of Entry</label>
              <input value={form.port_of_entry} onChange={e => setForm(f => ({ ...f, port_of_entry: e.target.value }))} placeholder="e.g. YYZ" className={inputCls} />
            </div>
          </div>
          <div><label className="block text-sm font-medium text-fg-secondary mb-1">Notes</label>
            <textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} rows={2} className={inputCls} />
          </div>
          <div className="flex gap-2 pt-2">
            <button type="submit" disabled={saving} className="px-5 py-2 rounded-full gradient-bg text-white text-sm font-medium disabled:opacity-50">
              {saving ? 'Saving...' : 'Save'}
            </button>
            <button type="button" onClick={() => setShowForm(false)} className="px-5 py-2 rounded-full border border-border text-fg-secondary text-sm">Cancel</button>
          </div>
        </form>
      )}
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <div className="mt-3 h-2 rounded-full bg-bg-tertiary overflow-hidden">
                  <div
                    className={clsx('h-full rounded-full transition-all', days <= 90 ? 'bg-danger' : days <= 180 ? 'bg-warning' : 'bg-success')}
                    style={{ width: `${Math.max(0, Math.min(100, (days / 1095) * 100))}%` }}
                  />
                </div>
                {days > 0 && days <= 180 && (
                  <Link href="/renewal" className="inline-flex items-center gap-2 mt-4 px-4 py-2 rounded-full gradient-bg text-white text-sm font-medium">
      ...
      {statuses.length === 0 && !showForm && (
        <div className="card p-10 text-center">
          ...
          <p className="text-fg-muted text-sm mt-1">Add your TN visa details to get renewal reminders.</p>
        </div>
      )}
          ...
          <div className="space-y-2">
            {statuses.slice(1).map(s => (
              <div key={s.id} className="card p-4 flex items-center justify-between">
                <div>
                  <p className="font-medium text-fg text-sm">{s.profession} at {s.employer}</p>
                  <p className="text-xs text-fg-muted">{s.start_date} to {s.end_date}</p>
                </div>
                <span className={clsx('text-xs font-medium', getDaysLeft(s.end_date) > 0 ? 'text-success' : 'text-fg-muted')}>
