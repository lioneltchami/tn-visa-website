...
describe("parseJobDescription", () => {
	...
	it("structures the RN employer blob into facts, sections, and requirements", () => {
		...
		if (facts?.type === "facts") {
			...
			expect(
				facts.facts.find((f) => f.label.toLowerCase() === "job type")?.value,
			).toMatch(/full-time/i);
			expect(
				facts.facts.find((f) => f.label.toLowerCase() === "location")?.value,
			).toMatch(/taos/i);
		}
	...
	it("keeps short clean descriptions as paragraphs", () => {
		...
		expect(
			parsed.blocks.every((b) => b.type === "paragraph" || b.type === "facts"),
		).toBe(true);
...
describe("extractJobRequirements", () => {
	it("prefers highlight items when present", () => {
		expect(
			extractJobRequirements(RN_BLOB, [
				"Active RN license",
				"BLS certification",
			]),
		).toEqual(["Active RN license", "BLS certification"]);
	});
